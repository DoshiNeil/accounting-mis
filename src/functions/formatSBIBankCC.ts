...
export function promptForSBICCNumber(): void {
  const ui = SpreadsheetApp.getUi();
  const accNumberField = ui.prompt("Account Number", "Enter last 4 digits of the CC", ui.ButtonSet.OK_CANCEL);

  // Process the user's accNumberField.
  if (accNumberField.getSelectedButton() == ui.Button.OK) {
    formatSBIBankCC(`SBICC${accNumberField.getResponseText()}`);
  }
}

export function formatSBIBankCC(sheetName: string): void {
  ...
  const transformedValues = values.map((row, idx) => {
    return [idx + 1, row[0], row[1], "", !!row[4] ? row[2] : "", !!row[3] ? row[2] : "", row[2], sheetName];
  });
